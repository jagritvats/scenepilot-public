...
| `SCENEPILOT_PAID_CALL_BUDGET=80` | The URL sits on a public Devpost page for weeks with no auth, and every paid feature is one unauthenticated POST away. Past the ceiling a call that *has* a recording behind it degrades to replay — same `REPLAY` status and "(replayed)" labels a replay-mode run produces, carrying the refusal as its stated reason — so the demo keeps working and keeps saying why. A live Monitor is refused outright (501): it is a billing object on Parallel's side and no recording can stand in for one. |
